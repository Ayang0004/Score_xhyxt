...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页管理 <span class="c-gray en">&gt;</span> 轮播管理 <span class="c-gray en">&gt;</span> 图片列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	...
	<span class="l">
	<butten class="btn btn-danger radius" type="butten" <a href="#" onclick="formSubmit('/picDelete','_self');this.blur();"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除 </a></butten>
	</span> 
	&nbsp;
	<span><a class="btn btn-primary radius" onclick="picture_add('添加图片','banner_admin_add')" href="/banner_admin_add"><i class="Hui-iconfont">&#xe600;</i> 添加图片</a>
	</span>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<tr class="text-c">
					<td><input name="id" type="checkbox" value="${b.id}"></td>
					<td>${b.picTitle}</td>
					<td>${b.picName }</td>
					<td>${b.picCategory }</td>
					<td>${b.id }</td>
					<td><fmt:formatDate value="${b.created}" pattern="yyyy-MM-dd" /></td>
					<td><fmt:formatDate value="${b.updated}" pattern="yyyy-MM-dd" /></td>
					...
					<a style="text-decoration:none" class="ml-5"  href="/tobannerPicUpdate?id=${b.id}" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
					<a style="text-decoration:none" class="ml-5"  href="/picDelete?id=${b.id}" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
			</tbody>
				</c:forEach>
		</table>
...
function formSubmit (url,sTarget){
    ...
    var checked=false;
	var ids= document.getElementsByName("projectId");
...
function formSubmit2 (url,projectId){
	if (confirm ("您确定执行该操作吗?")){		
		$("#paging").load("${ctx}/pages/project/"+url,{projectId:projectId});
  	}
    return true;
}
